...
def match(expectedSymbol, lexical):
  ...
  if token != expectedSymbol: 
    raise SyntacticError(token, expectedSymbol)

def asd(nonterminal, final_grammar, lexical):
  firstSymbol = lexical.get_current_token()
  if firstSymbol == None: print("----> syntactic error") 
  
  rules = final_grammar.rules[nonterminal].rules
  selectedRule = list(filter(lambda rule : firstSymbol.t_type in rule.prediction_set, rules))
  if len(selectedRule) > 1: print("----> grammar error")
  if len(selectedRule) == 0:
    predictions = {symbol for rule in rules for symbol in rule.prediction_set}
    raise SyntacticError(firstSymbol.t_type, predictions, firstSymbol.row, firstSymbol.column)

  right_part = selectedRule[0].right_part
  if right_part == ['epsilon']: right_part = []
