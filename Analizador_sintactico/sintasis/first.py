...
def first_aux(grammar, noTerminalSymbol, rules):
    print(type(rules))
    for rule in rules:  # analiza regla por regla
        if rule.right_part[0] == "epsilon":
            grammar[noTerminalSymbol].put_first("epsilon")
        else:
            for symbol in rule.right_part:
                if symbol not in grammar:
                    grammar[noTerminalSymbol].put_first(symbol)
                    break
                elif symbol in grammar:
                    if grammar[symbol].first == []:
                        symbolRules = grammar.get(symbol).rules
                        first_aux(grammar, symbol, symbolRules)
                    symbolFirsts = grammar[symbol].first
                    addToTerminal = set(symbolFirsts).difference("epsilon")
                    grammar[noTerminalSymbol].put_first(
                        list(addToTerminal))
                    if "epsilon" in symbolFirsts:
...
def first(gram):
    ...
    for noTerminalSymbol, rules_1 in grammar.items():
        if grammar[noTerminalSymbol].first == []:
            first_aux(grammar, noTerminalSymbol, rules_1.rules)
    return grammar
